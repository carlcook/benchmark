...
double callOptionPrice(int S, double t, int X, double r, double sigma, double T)
{
...
void CalcCallOption(benchmark::State& state) {
    while (state.KeepRunning()) {
	benchmark::DoNotOptimize(callOptionPrice(state.range(0), 0, state.range(1), 0.05, 0.2, 1));
    }
}

static void CustomArguments(benchmark::internal::Benchmark* b) {
  for (int strike = 10; strike < 20; ++strike)
   for (int underlying = 1; underlying < 10; ++underlying)
      b->Args({strike, underlying});
}

BENCHMARK(CalcCallOption)->Apply(CustomArguments);

BENCHMARK(CalcCallOption);
